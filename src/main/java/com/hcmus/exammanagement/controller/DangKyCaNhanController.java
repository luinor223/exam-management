...
public class DangKyCaNhanController {
    ...
    void btnDangKyThi(ActionEvent event) {
        try {
            ...
            // Save the customer
            KhachHangDTO khachHangTonTai = KhachHangBUS.layKhachHangBangCCCD(khachHang.getCccd());
            if (khachHangTonTai == null) {
                khachHang = KhachHangBUS.taoKhachHang(khachHang);
            } else {
                khachHang = khachHangTonTai;
            }
            ...
            // Save the candidate
            ThiSinhDTO thiSinhTonTai = ThiSinhBUS.layThiSinhBangCCCD(thiSinh.getCccd());
            if (thiSinhTonTai == null) {
                thiSinh = ThiSinhBUS.taoThiSinh(thiSinh);
            } else {
                thiSinh = thiSinhTonTai;
            }
            ...
            PhieuDangKyDTO phieuDangKy = new PhieuDangKyDTO(
                ...
                "", // diaChiGiao is empty for individual
                    khachHang, // maKH
                null // maNVTao will be generated by the database
            );
